...
import { wait, hitStaticResource, loadIndexHtml, extractScriptAndStyleUrlsFromHtml, resultHasError } from './utils.js';
...
async function loadTest(url) {
    ...
    const staticResourcesUrls = extractScriptAndStyleUrlsFromHtml(pageHtmlString);
    const staticFilesFetchResults = await Promise.allSettled(
        staticResourcesUrls.map((file) => hitStaticResource(`${config.BASE_DOMAIN}/${file}`)),
    );
    ...
    if (resultHasError(result)) {
        ...
        console.warn(
            LOG_PREFIX +
                `${result.timestampStart},${result.timestampEnd},${good.length},${non200.length},${
                    result.rootResponse
                },${non200.join(';')}`,
        );
    } else {
