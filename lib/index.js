...
async function loadTest(url) {
    ...
    if (resultHasError(result)) {
        ...
        console.warn(
            LOG_PREFIX + `${result.timestampStart}-${result.timestampEnd}: hit ${non200.length} non-200 requests, (main: ${result.rootResponse}, sub: ${non200.join(", ")})`,
        );
    } else {
        console.info(
            LOG_PREFIX +
                `${result.timestampStart}-${result.timestampEnd}: got ${Object.values(result.subPages).length} 200 OKs`,
        );
    }
...
// Stagger processes 300ms each
console.info(`Staggering process ${PROCESS_INDEX} ${PROCESS_INDEX * 300}ms`);
await wait(PROCESS_INDEX * 300);
